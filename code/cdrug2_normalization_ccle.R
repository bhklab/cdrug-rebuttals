...
if(!file.exists(myfn)) {
  ...
  myfn2 <- file.path(path.ge, "dwl", sprintf("%s.tar.gz", archivn))
  if (!file.exists(myfn2)) {
    dwl.status <- download.file(url=sprintf("http://www.broadinstitute.org/ccle/downloadFile/DefaultSystemRoot/exp_10/ds_21/%s.tar.gz?downloadff=true&fileId=9619", archivn), destfile=file.path(path.ge, "dwl", sprintf("%s.tar.gz", archivn)), quiet=TRUE)
    if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
  }
  ## untar
  res <- untar(tarfile=file.path(path.ge, "dwl", sprintf("%s.tar.gz", archivn)), exdir=path.ge)
  
  fff <- affy::list.celfiles(file.path(path.ge, archivn))
  res <- apply(cbind("from"=file.path(path.ge, archivn, fff), "to"=file.path(path.ge, fff)), 1, function(x) { return(file.rename(from=x[1], to=x[2])) })
  unlink(file.path(path.ge, archivn), recursive=TRUE)
  
  celfile.timestamp <- as.character(file.info(file.path(path.ge, fff))[, "mtime"])
  ...
  rr <- sapply(file.path(path.ge, fff), R.utils::gzip, overwrite=TRUE)
...
dir.create(file.path(path.cell, "dwl"), showWarnings=FALSE)
myfn <- file.path(path.cell, "ccle_sample_info_file.txt")
if (!file.exists(myfn)) {
  dwl.status <- download.file(url="http://www.broadinstitute.org/ccle/downloadFile/DefaultSystemRoot/exp_10/ds_22/CCLE_sample_info_file_2012-10-18.txt?downloadff=true&fileId=6801", destfile=file.path(path.cell, "dwl", "CCLE_sample_info_file_2012-10-18.txt"), quiet=TRUE)
  if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
  file.copy(from=file.path(path.cell, "dwl", "CCLE_sample_info_file_2012-10-18.txt"), to=myfn)
}
  
 ## drug info
message("Download drug information")
dir.create(file.path(path.drug, "dwl"), showWarnings=FALSE)
myfn <- file.path(path.drug, "ccle_drug_info_file.csv")
if (!file.exists(myfn)) {
  dwl.status <- download.file(url="http://www.broadinstitute.org/ccle/downloadFile/DefaultSystemRoot/exp_10/ds_27/CCLE_NP24.2009_profiling_2012.02.20.csv?downloadff=true&fileId=3422", destfile=file.path(path.drug, "dwl", "CCLE_NP24.2009_profiling_2012.02.20.csv"), quiet=TRUE)
  if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
  file.copy(from=file.path(path.drug, "dwl", "CCLE_NP24.2009_profiling_2012.02.20.csv"), to=myfn)
}
  
 ## drug pheno
message("Download drug sensitivity measurements")
myfn <- file.path(path.drug, "ccle_drug_pheno_file.xls")
if (!file.exists(myfn)) {
  ## drug sensitivity data from the addendum in Nature
  dwl.status <- download.file(url="http://www.nature.com/nature/journal/v492/n7428/extref/nature11735-s2.xls", destfile=file.path(path.drug, "dwl", "nature11735-s2.xls"), quiet=TRUE)
  if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
  file.copy(from=file.path(path.drug, "dwl", "nature11735-s2.xls"), to=myfn)
}
## drug sensitivity data from the CCLE website
# dwl.status <- download.file(url="http://www.broadinstitute.org/ccle/downloadFile/DefaultSystemRoot/exp_10/ds_27/CCLE_NP24.2009_Drug_data_2012.02.20.csv?downloadff=true&fileId=2114", destfile=file.path(path.drug, "dwl", "CCLE_NP24.2009_Drug_data_2012.02.20.csv"), quiet=TRUE)
# if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
# file.copy(from=file.path(path.drug, "dwl", "CCLE_NP24.2009_Drug_data_2012.02.20.csv"), to=file.path(path.drug, "ccle_drug_pheno_file.csv"))

## mutations
message("Download mutation data")
## hybrid capture
dir.create(file.path(path.mut, "dwl"), showWarnings=FALSE)
myfn <- file.path(path.mut, "ccle_mutations_hybrid.maf")
if (!file.exists(myfn)) {
  dwl.status <- download.file(url="http://www.broadinstitute.org/ccle/downloadFile/DefaultSystemRoot/exp_10/ds_26/CCLE_hybrid_capture1650_hg19_NoCommonSNPs_NoNeutralVariants_CDS_2012.05.07.maf.gz?downloadff=true&fileId=6873", destfile=file.path(path.mut, "dwl", "CCLE_hybrid_capture1650_hg19_NoCommonSNPs_NoNeutralVariants_CDS_2012.05.07.maf.gz"), quiet=TRUE)
  if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
  gunzip(filename=file.path(path.mut, "dwl", "CCLE_hybrid_capture1650_hg19_NoCommonSNPs_NoNeutralVariants_CDS_2012.05.07.maf.gz"), overwrite=TRUE)
  file.copy(from=file.path(path.mut, "dwl", "CCLE_hybrid_capture1650_hg19_NoCommonSNPs_NoNeutralVariants_CDS_2012.05.07.maf"), to=myfn)
}
## oncomap3
myfn <- file.path(path.mut, "ccle_mutations_oncomap.maf")
if (!file.exists(myfn)) {
  dwl.status <- download.file(url="http://www.broadinstitute.org/ccle/downloadFile/DefaultSystemRoot/exp_10/ds_23/CCLE_Oncomap3_2012-04-09.maf?downloadff=true&fileId=3000", destfile=file.path(path.mut, "dwl", "CCLE_Oncomap3_2012-04-09.maf"), quiet=TRUE)
  if(dwl.status != 0) { stop("Download failed, please rerun the pipeline!") }
  file.copy(from=file.path(path.mut, "dwl", "CCLE_Oncomap3_2012-04-09.maf"), to=myfn)
}
...
if(!file.exists(myfn)) {
  ...
  druginfo <- read.csv(file.path(path.drug, "ccle_drug_info_file.csv"))
  ...
  ## drug sensitivity data from the CCLE website
  # drugpheno <- read.csv(file.path(path.drug, "ccle_drug_pheno_file.csv"))
  ...
  sampleinfo <- read.csv(file.path(path.cell, "ccle_sample_info_file.txt"), sep="\t")
  ...
  if(!file.exists(myfn2)) {
    ...
    ## from hybrid capture
    mut <- read.csv(file.path(path.mut, "ccle_mutations_hybrid.maf"), sep="\t")
    ...
    ## from oncomap
    mut2 <- read.csv(file.path(path.mut, "ccle_mutations_oncomap.maf"), sep="\t")
  ...
  myfn2 <- file.path(saveres, "ccle_ge_norm.RData")
  if(!file.exists(myfn2)) {
    ...
    rownames(genexprs) <- rownames(sampleinfo)[match(rownames(genexprs), as.character(sampleinfo[ ,"filename"]))]
    save(list=c("genexprs"), compress=TRUE, file=myfn2)
  } else { load(myfn2) }

  ## build annotation matrix
  message("Build annotation matrix")
  myfn2 <- file.path(saveres, "ccle_ge_annot.RData")
  if(!file.exists(myfn2)) {
